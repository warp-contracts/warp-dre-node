...
const {
    createNodeDbTables, insertFailure, upsertBlacklist, getFailures, connect, events, hasContract, connectEvents,
...
async function runListener() {
    ...
    updateEvents.on('completed', async ({jobId, returnvalue}) => {
        ...
        const contractTxId = jobId.split("|")[0];if (returnvalue?.lastSortKey) {
...
async function subscribeToGatewayNotifications(nodeDb, nodeDbEvents, updatedQueue, registerQueue) {
    const onMessage = async (data) =>
        await processContractData(data, nodeDb, nodeDbEvents, registerQueue, updatedQueue);
